...
Layout.prototype.setPointScalar = function() {
  ...
  if (size) {
    world.elems.pointSize.value = size; //config.size.points.initial //size
    world.setUniform('scaleTarget', world.getPointScale());
    document.getElementById("pointsize-range-input").value = size;
  }
}
...
function World() {
  this.canvas = document.querySelector('#sflmt-canvas');
...
World.prototype.getShaderMaterial = function(obj) {
  ...
  return new THREE.RawShaderMaterial({
    uniforms: {
      ...
      borderColor: {
        type: 'vec3',
        value: new Float32Array([230/255, 1/255, 0/255]),
      },
      delay: {
        type: 'f',
...
World.prototype.render = function() {
  ...
  //console.log(world.elems.pointSize.value);
}
...
Picker.prototype.onMouseUp = function(e) {
  ...
  if (click.x !== this.mouseDown.x ||
      ...
      e.target.id !== 'sflmt-canvas') { // whether the click hit the gl canvas
    return;
  }
...
function getCanvasSize() {
  var elem = document.querySelector('#sflmt-canvas');
  return {
    w: elem.clientWidth,
    h: elem.clientHeight,
  }
